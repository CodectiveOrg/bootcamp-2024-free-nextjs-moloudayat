...
export default function ItemsProvider({
  ...
  const isActive = useCallback(
    (item: ItemType): boolean => {
      if (filters.even && item.value % 2 === 0) return true;
      if (filters.odd && item.value % 2 === 1) return true;
      if (filters.three && item.value % 3 === 0) return true;
      if (filters.five && item.value % 5 === 0) return true;
